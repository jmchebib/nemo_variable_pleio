/**  $Id: LCEcomposite.cc,v 1.20 2016-05-02 12:07:00 fred Exp $
 *
 ...
 *  @author fred
 */
#include <algorithm>   //for 'find' on vector
...
bool LCE_Breed_Selection::setParameters ( )
{
  ...
  if(getMeanFecundity(0) <= 0)
    return error("\"mean_fecundity\" is not set!\n");
                                                             ...
                                                             unsigned int natalpatch)
{
  ...
  register double fitness = getFitness(ind, natalpatch);
  register unsigned int cat = ind->getPedigreeClass();
