/**  $Id: LCEselection.cc,v 1.25 2016-10-31 15:18:35 fred Exp $
*
...
void LCE_Selection_base::loadFileServices ( FileServices* loader )
{
  if(_paramSet->isSet("selection_output")) {
    ...
    if(!param->isSet()) fatal("parameter \"selection_output_logtime\" is missing\n");
...
#ifdef _DEBUG_
      cout<<"#### attaching trait \""<<traits[i].c_str()<<"\" to selection LCE, index = "<<_TraitIndices[i]<<endl;
#endif
...
{  
  ...
  if(_paramSet->isSet("selection_fitness_model")) {
    ...
    } else if(fit_model.compare("relative_max_local") == 0) { 
    ...
    } else if(fit_model.compare("relative_max_global") == 0) {
...
{  
  ...
  // SET FUNCTION POINTERS TO FITNESS FUNCTIONS
  ...
  for(unsigned int t = 0; t < _SelectionModels.size(); t++) {
    ...
    } else return error("wrong selection model, must be either \"fix\", \"direct\", \"quadratic\", or \"gaussian\".\n");
...
double LCE_Selection_base::getMaxFitness (age_idx age)
{
  ...
  for(unsigned int i = 0, npatch = _popPtr->getPatchNbr(); i < npatch; i++) {
    ...
    local_max = getMaxPatchFitness(age, i);
...
// ----------------------------------------------------------------------------------------
double LCE_Selection_base::getMaxPatchFitness (age_idx age, unsigned int p)
{
  double max = 0, fit;
  Patch *patch = _popPtr->getPatch(p);
  
  for(unsigned int j = 0, size = patch->size(FEM, age); j < size; j++) {
    fit = getFitness( patch->get(FEM, age, j), p);
    max = (fit > max ? fit : max);
  }
  for(unsigned int j = 0, size = patch->size(MAL, age); j < size; j++){
    fit = getFitness( patch->get(MAL, age, j), p);
    max = (fit > max ? fit : max);
  }
...
// ----------------------------------------------------------------------------------------
void LCE_Selection_base::setScalingFactorMaxLocal (age_idx age, unsigned int p)
{
  _scaling_factor = 1; //this to have the raw fitness when computing fitnesses below
  _scaling_factor = 1.0/getMaxPatchFitness(age, p);
}
// ----------------------------------------------------------------------------------------
// LCE_Selection_base::setScalingFactorGlobal
// ----------------------------------------------------------------------------------------
void LCE_Selection_base::setScalingFactorMaxGlobal (age_idx age, unsigned int p)
{
  if(p != 0) return; //we compute the max fitness only once
  _scaling_factor = 1;
  _scaling_factor = 1.0/getMaxFitness(age);
}
