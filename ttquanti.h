...
public:
  ...
  TTQuanti () : _sequence(0),_phenotypes(0),_nb_locus(0),_nb_traits(0),_seq_length(0),
                _genomic_mutation_rate(0),_init_value(0),_myProto(0),
...
//  virtual   void*           get_allele (int loc, int all) const     {return (loc < (int)_seq_length && all < 2 ? (void*)&_sequence[all][loc] : 0);}
  ...
  {_mutationFuncPtr = (_isHC ? &TTQuanti::mutate_HC : &TTQuanti::mutate_noHC); }
...
private:    
  ...
  vector< double* (TProtoQuanti::* ) (unsigned int) > _mutation_func_ptrs;
