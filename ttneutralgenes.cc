/**  $Id: ttneutralgenes.cc,v 1.28 2016-03-03 15:23:08 fred Exp $
 *
 *  @file ttneutralgenes.cc
 *  Nemo2
 *
 *   Copyright (C) 2006-2010 Frederic Guillaume
...
_stats(0), _type(NTRL)
{
	...
	add_parameter("ntrl_init_model",        INT, false, true, 0, 2);
...
// set_pleio_sequence
// ----------------------------------------------------------------------------------------
//void TTNeutralGenes::set_pleio_sequence(void** seq)
//{
//	reset(); init();
//	memcpy(_pleio_sequence[0],seq[0],_locus_num);
//	memcpy(_pleio_sequence[1],seq[1],_locus_num);
//}
// ----------------------------------------------------------------------------------------
// set_mutcor_sequence
// ----------------------------------------------------------------------------------------
//void TTNeutralGenes::set_mutcor_sequence(void** seq)
//{
//	reset(); init();
//	memcpy(_mutcor_sequence[0],seq[0],_locus_num);
//	memcpy(_mutcor_sequence[1],seq[1],_locus_num);
//}
// ----------------------------------------------------------------------------------------
// show_up
// ----------------------------------------------------------------------------------------
...
//  unsigned char** mother_pleio_seq = (unsigned char**)mother->get_pleio_sequence();
//	unsigned char** father_pleio_seq = (unsigned char**)father->get_pleio_sequence();

//    (_myProto->* _inherit) (FEM, _sequence[FEM], _pleio_sequence[FEM], mother_seq, mother_pleio_seq);
	(_myProto->* _inherit_func_ptr) (FEM, _sequence[FEM], mother_seq);
//	(_myProto->* _inherit_func_ptr) (FEM, _sequence[FEM], _pleio_sequence[FEM], mother_seq, mother_pleio_seq);
//    (_myProto->* _inherit) (MAL, _sequence[MAL], _pleio_sequence[MAL], mother_seq, mother_pleio_seq);
	(_myProto->* _inherit_func_ptr) (MAL, _sequence[MAL], father_seq);
//	(_myProto->* _inherit_func_ptr) (MAL, _sequence[MAL], _pleio_sequence[MAL], father_seq, father_pleio_seq);
}
// ----------------------------------------------------------------------------------------
// mutate_SSM
// ----------------------------------------------------------------------------------------
...
{    
	...
	if(AGE == (ADULTS | OFFSPRG) || AGE == ALL) {
		warning("saving only offspring neutral allele frequencies\n");
		AGE = OFFSPRG;
	}
	...
	//do this here, population may have been resized...
	stater->allocateTables(nb_locus, nb_allele);

	stater->setAlleleTables(AGE);
	stater->setHeteroTable (AGE);

	DataTable<double> *freqTable = stater->getAlleleFreqTable();
	DataTable<double> *hTable = stater->getHeteroTable();

	std::string filename = get_path() +
			this->get_service()->getGenerationReplicateFileName();
	string file1 = filename + get_extension();
	string file2 = filename + ".Ho";
	string file3 = filename + ".He";

	unsigned int *sizes = new unsigned int [patchNbr];

	for(unsigned int i = 0; i < patchNbr; ++i)
		sizes[i] = _pop->size(AGE, i);

#ifdef _DEBUG_
	message("TTNeutralGenesFH::write_freq (%s)\n",filename.c_str());
#endif

	ofstream FILE1 (file1.c_str(), ios::out);
	ofstream FILE2 (file2.c_str(), ios::out);
	ofstream FILE3 (file3.c_str(), ios::out);

	if(!FILE1) fatal("could not open neutral allele freq output file!!\n");
	if(!FILE2) fatal("could not open neutral allele heterosygosity output file!!\n");
	if(!FILE3) fatal("could not open neutral allele heterosygosity output file!!\n");
	...
	FILE1<<endl; FILE2<<endl; FILE3<<endl;
	...
	for(unsigned int l = 0; l < nb_locus; ++l) {
		for(unsigned int i = 0; i < patchNbr; ++i) {
			p2 = 0;
			for(unsigned int u = 0; u < nb_allele; u++){
				p = freqTable->get(i, l, u);
				FILE1<< p << " ";
				p2 += p*p; //He = 1 - sum of p2
			}
			FILE2<<hTable->get(i, l, 0)/sizes[i]<<" ";
			FILE3<<1-p2<<" ";
		}
		FILE1<<endl; FILE2<<endl; FILE3<<endl;
	}
