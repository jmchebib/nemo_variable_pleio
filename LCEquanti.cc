/** $Id: LCEquanti.cc,v 1.14 2016-03-03 15:23:50 fred Exp $
 *
 *  @file LCEquanti.cc
 *  Nemo2
 *
...
void LCE_QuantiInit::execute ( )
{
  ...
  if (_doByTraitValue) {
    ...
    for (unsigned int i = 0; i < patchNbr ; i++) { 
      ...
      init_trait_value(FEM, OFFSx, _popPtr->size(FEM, OFFSPRG, i), i, values);
      init_trait_value(MAL, OFFSx, _popPtr->size(MAL, OFFSPRG, i), i, values);
      init_trait_value(FEM, ADLTx, _popPtr->size(FEM, ADULTS, i), i, values);
      init_trait_value(MAL, ADLTx, _popPtr->size(MAL, ADULTS, i), i, values);
    }
  }
  ...
  if (_doByAlleleFreq) {
    ...
    for (unsigned int i = 0; i < patchNbr ; i++) { 
      ...
      init_allele_freq(FEM, OFFSx, _popPtr->size(FEM, OFFSPRG, i), i, values, allele_val);
      init_allele_freq(MAL, OFFSx, _popPtr->size(MAL, OFFSPRG, i), i, values, allele_val);
      init_allele_freq(FEM, ADLTx, _popPtr->size(FEM, ADULTS, i), i, values, allele_val);
      init_allele_freq(MAL, ADLTx, _popPtr->size(MAL, ADULTS, i), i, values, allele_val);
    }
  }
...
// ----------------------------------------------------------------------------------------
void LCE_QuantiInit::init_allele_freq(sex_t SEX, age_idx age, unsigned int size, unsigned int deme, double *values, double **all_val)
{ 
  if(!size) return;
...
void LCE_NtrlInit::execute ( )
{
  ...
  double *values = new double[_nLoci];
  
  for (unsigned int i = 0; i < patchNbr ; i++) { 
    
    _init_freq.getRowView(i, _nLoci, values);  
    
    init_allele_freq(FEM, OFFSx, _popPtr->size(FEM, OFFSPRG, i), i, values);
    init_allele_freq(MAL, OFFSx, _popPtr->size(MAL, OFFSPRG, i), i, values);
    init_allele_freq(FEM, ADLTx, _popPtr->size(FEM, ADULTS, i), i, values);
    init_allele_freq(MAL, ADLTx, _popPtr->size(MAL, ADULTS, i), i, values);
  }
...
// ----------------------------------------------------------------------------------------
void LCE_NtrlInit::init_allele_freq(sex_t SEX, age_idx age, unsigned int size, unsigned int deme, double *values)
{ 
  if(!size) return;
