/**  $Id: ttdeletmutations_bitstring.cc,v 1.27 2016-10-31 14:38:35 fred Exp $
 *
...
bool TProtoDeletMutations_bitstring::setSelectionParameters ()
{
  ...
  //for backward compatibility only, param removed
...
// get_allele_value
// ----------------------------------------------------------------------------------------
double TTDeletMutations_bitstring::get_allele_value (int loc, int all)      
{
  if((*sequence[all])[loc]) {
    if(get_hmz_atLocus(loc)) return _effects[1][loc];
    else return _effects[0][loc];
  } else {
    if(get_hmz_atLocus(loc)) return 0;
    else return _effects[0][loc];
  }
}
...
// set_allele_value
// ----------------------------------------------------------------------------------------
void TTDeletMutations_bitstring::set_allele_value (unsigned int locus, unsigned int allele, double value)
{
  assert(locus < _nb_locus && allele < 2); 
  
  sequence[allele]->set(locus); 

  _effects[1][locus] = (float)value;
  
  double dom;
  if(_myProto->get_dominance_model() == 1)
    dom = exp(-1.0*_effects[1][locus]*(-1.0*log(2*_dominance) / _strength))/2.0; //scaling of h on s
  else dom = _dominance;
  
  _effects[0][locus] = (float)(dom * _effects[1][locus]); //heterozygote effect: hs
...
void TTDeletMutBitstrFH::FHwrite()
{
  ...
  FILE<<"age sex ped origin ID"<<endl;
  ...
  for (int i = 0; i < patchNbr; ++i) {
    ...
    for (unsigned int j = 0, size = current_patch->size(FEM, OFFSx); j < size; ++j) {
      ...
      FILE << OFFSPRG << " " << FEM << " " << ind->getPedigreeClass() << " " 
           << ind->getHome()<<" "<<ind->getID()<<endl;
    }
    ...
    for (unsigned int j = 0, size = current_patch->size(MAL, OFFSx); j < size; ++j) {
      ...
      FILE << OFFSPRG << " " << MAL << " "  << ind->getPedigreeClass() << " " 
           << ind->getHome()<<" "<<ind->getID()<<endl;
    }
    ...
    for (unsigned int j = 0; j < current_patch->size(FEM, ADLTx); ++j) {
      ...
      FILE << ADULTS << " " << FEM << " " << ind->getPedigreeClass() << " "
           << ind->getHome()<<" "<<ind->getID()<<endl;
    }
    ...
    for (unsigned int j = 0; j < current_patch->size(MAL, ADLTx); ++j) {
      ...
      FILE << ADULTS << " " << MAL << " " << ind->getPedigreeClass() << " "
           << ind->getHome()<<" "<<ind->getID()<<endl;
    }
  }
...
void TTDeletMutBitstrFH::FHread (string& filename)
{
  ...
  if(!FILE) fatal("could not open DELET input file \"%s\"\n",filename.c_str());
