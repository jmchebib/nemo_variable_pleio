/**  $Id: statservices.cc,v 1.10 2015-07-13 08:52:58 fred Exp $
 *
...
bool StatServices::init() 
{
  ...
  list<string>::iterator token = args.begin();
...
{
  map<unsigned int, unsigned int>::const_iterator time = timeTable.begin(), next;
...
void StatServices::printStatHeaders(ofstream& FH)
{
  for (rec_it IT = _statRecorders.begin(); IT != _statRecorders.end(); ++IT) {
    FH << _separator << setw(_width) << (*IT)->getName();
  }
}
//-----------------------------------------------------------------------------------------
// printStatValue
// ----------------------------------------------------------------------------------------
void StatServices::printStatValue(ofstream& FH, unsigned int repl_idx)
{
...
//  FH.setf(ios::left,ios::adjustfield);
  ...
  if(SIMenv::getCurrentReplicate() == 1) {
    ...
    FH << setw(_width) << left <<"replicate"<< _separator << setw(_width) <<"generation";
...
//    FH.width(_width);
    FH << setw(_width) << left << _statValues[repl_idx][gen][0]; //replicate number
    FH << _separator;
    FH << setprecision(16); //to avoid scientific notation for generation number
    FH << setw(_width) << _statValues[repl_idx][gen][1]; //generation recorded
    FH << setprecision(_precision); //this precision is needed only for stat values
    
    i = 2;
    //print all stats on one line
    for (rec_it IT = _statRecorders.begin(); IT != _statRecorders.end(); ++IT) {
      
      FH << _separator << setw(_width) << _statValues[repl_idx][gen][i];
...
//  FH.setf(ios::left,ios::adjustfield);
  FH << setw(_width) << left <<"generation";
  
  //print the stat names:
  printStatHeaders(FH);
  
  FH<< _separator << setw(_width) << "alive.rpl" << endl;
...
//      FH.setf(ios::left,ios::adjustfield);
      ...
      FH << setw(_width) << left << current_gen;
      
      FH << setprecision(_precision);
      
      for (unsigned int j = 0; j < _numRecorders; ++j) {
        FH << _separator  << setw(_width) << means[j] / rep_cntr;
      }
      //print number of alive replicates 'til this generation
      FH << _separator << setw(_width) << rep_cntr;
