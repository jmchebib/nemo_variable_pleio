/**  $Id: ttneutralmito.h,v 1.7 2016-03-03 15:20:13 fred Exp $
*
...
#ifndef TTNEUTRALMITO_H
#define TTNEUTRALMITO_H
...
class TTNeutralMitoFH;
//class TTNeutralGenesSH;
class TProtoNeutralMito;
// ----------------------------------------------------------------------------------------
...
class TTNeutralMito : public TTrait 
{
private:
  TProtoNeutralMito* _myProto;
  ...
  double _mean_mut_num;
  unsigned short _init_model;
  void (TTNeutralMito::* _mutate_func_ptr) (void);
...
public:
  ...
  TTNeutralMito () 
    : _myProto(0), _allele_num(0), _locus_num(0), _mut_rate(0), _mut_model(0),
    _mean_mut_num(0),_init_model(0), _mutate_func_ptr(0), _sequence(0), _type("mito") { }
  
  TTNeutralMito(const TTNeutralMito& T) 
    : _myProto(T._myProto),
    _allele_num(T._allele_num), _locus_num(T._locus_num), _mut_rate(T._mut_rate),
    _mut_model(T._mut_model), _mean_mut_num(T._mean_mut_num),_init_model(T._init_model), 
    _mutate_func_ptr(T._mutate_func_ptr), _sequence(0), _type("mito") { }
  
  virtual ~TTNeutralMito ();
  ...
  unsigned char*  getHaploSequence     ( )                     {return _sequence;}
  
  void            set_proto          (TProtoNeutralMito* proto) {_myProto = proto;}
  ...
  void            set_mean_mut_num     (double value)          {_mean_mut_num = value;}
  void            set_mut_model        (int value)             {_mut_model = value;}
  void            set_init_model       (unsigned short val)    {_init_model = val;}
  void            set_mut_func_ptr     (void(TTNeutralMito::* theFunc)(void))
  ...
  ///@{
  virtual TTNeutralMito& operator= (const TTrait& T);
  ...
  virtual double   get_allele_value    (int loc, int all)  
    {return ( !(loc<(int)_locus_num) ? 0 : _sequence[loc]);}
  virtual void    set_allele_value     (unsigned int locus, unsigned int allele, double value)  
    {assert (locus<_locus_num); _sequence[locus] = value;}
  ...
  virtual TTNeutralMito*  clone        ( )                      {return new TTNeutralMito(*this);}
  
  //implements StorableComponent
  virtual void    store_data       (BinaryStorageBuffer* saver)  
  {
    for(unsigned int i = 0; i < _locus_num; ++i) saver->store(&_sequence[i], 1);
  }
  virtual bool    retrieve_data    (BinaryStorageBuffer* reader)
  { 
    for(unsigned int i = 0; i < _locus_num; ++i) reader->read(&_sequence[i], 1);
    return true;
  }
  ///@}
};  
...
//                     N E U T R A L  mtD N A  P R O T O T Y P E

// ----------------------------------------------------------------------------------------
/**Prototype class for the TTNeutralMito trait class.**/
class TProtoNeutralMito : public TraitPrototype {
private:
  unsigned int _allele_num;
  unsigned int _locus_num;
  double _mut_rate;
...
public:
  ...
  TProtoNeutralMito(const TProtoNeutralMito& T);
  
  virtual ~TProtoNeutralMito ( );
  ...
  virtual TTNeutralMito*          hatch ();
  
  virtual TProtoNeutralMito*      clone () {return new TProtoNeutralMito(*this);}
...
/**A file handler to save the neutral markers genotypes in the FSTAT format (extended). 
   By default, the file extension is ".mtd" for the genotype file. 
*/
class TTNeutralMitoFH: public TraitFileHandler<TProtoNeutralMito> {
  
  void (TTNeutralMitoFH::* write_fct) ();
  
public:
	
  TTNeutralMitoFH ( TProtoNeutralMito* TP )
    : TraitFileHandler<TProtoNeutralMito> (TP, ".mtd")
  { }
  
  virtual ~TTNeutralMitoFH ( ) { }
...
//  void write_varcompWC();
  ...
  void set_write_fct( void (TTNeutralMitoFH::* fct_ptr) () ) {write_fct = fct_ptr;}
  
};

/**The stat handler for neutral markers. */
// class TTNeutralGenesSH: public TraitStatHandler<TProtoNeutralMito, TTNeutralGenesSH> {
//  
//  DataTable< unsigned int > _alleleCountTable;
//  DataTable< double > _alleleFreqTable;
//  DataTable< unsigned int > _heteroTable;
...
//  double _fst_WH, _fst_WC, _fis_WC, _fit_WC;
...
//public:
//  
//	TTNeutralGenesSH (TProtoNeutralMito* TP) 
//    : TraitStatHandler<TProtoNeutralMito, TTNeutralGenesSH>(TP), _table_set_gen(0), _table_set_age(0),
//    _table_set_repl(0), _coa_matrix(0), _fst_matrix(0), _D(0) { }
//  
...
//  
//  DataTable<unsigned int>*   getHeteroTable      () {return &_heteroTable;}
//  
